...
class WeightedSumPlusStub(nn.Module):
    """ Performs adaptive weighted-sum fusion to merge the input embeddings of the burst images """
    def __init__(self, input_dim, project_dim, offset_feat_dim,
                 num_offset_feat_extractor_res=1, num_weight_predictor_res=1, use_offset=True, offset_modulo=None,
                 ref_offset_noise=0.0, softmax=True, use_base_frame=False,
                 use_bn=False, activation='relu', upsample_factor=2, diffusion_cond_dims=3):
        super().__init__()
        self.use_offset = use_offset
        self.offset_modulo = offset_modulo
        self.ref_offset_noise = ref_offset_noise
        ...
        stub_upsampler = PixShuffleUpsampler(input_dim=input_dim, output_dim=diffusion_cond_dims, upsample_factor=upsample_factor, use_bn=use_bn, activation=activation, icnrinit=False, gauss_blur_sd=None, gauss_ksz=3)
    ...
    def forward(self, x):
        ...
        # Perform fusion
        fused_feat = (all_feat * weights_norm).sum(dim=1)
        fused_feat = stub_upsampler(fused_feat)  # 512, 80, 80 -> 3, 160, 160
        # 64 Channels -> 3 * upsample_factor^2 channels
        # Reshuffle -> 3, x * upsample_factor, y * upsample_factor
        out = {'fused_enc': fused_feat, 'fusion_weights': weights_norm}
        return out
